...
class zu3 extends StatefulWidget{
  @override
...
class zu3_ extends State<zu3>{
        ...
        Wrap(
          ...
          children: <Widget>[
            Container(
              ...
              child: Text("至少选择2个号，猜对开奖号(顺序不限)即中346元"),
            ),
            Container(
              child: Row(
                ...
                GestureDetector(
                  onTap: (){
                    if(int.parse(getZhu())>0){
                     JumpAnimation().jump(plOrder(zhu: int.parse(getZhu()),s1:s1,s2:s2,s3:s3,type: 2,), context);
                    }
                  },
                  child: Container(
  ...
  getZhu(){
    if(s1.length>1){
      return (s1.length*(s1.length-1)).toString();
    }else{
      return "0";
    }
